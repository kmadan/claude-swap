...
- **Strategies** (`--strategy`, or `cswap config set autoswitch.strategy`): `best` (default) stays put until the active account nears its limit, then moves to the account with the most quota left. `consume-first` proactively keeps you on the account whose **weekly window resets soonest** — use-it-or-lose-it — switching to a sooner-resetting account (with room to spare) even below the threshold, so perishable weekly quota isn't wasted. `runway` keeps you working: when a switch is due it takes the account with the most weekly headroom **per day until that headroom resets**, so an account holding a fresh week outranks one holding a handful of points it could never spend, and a nearly-spent account whose reset is hours away outranks both. Use it when running out of accounts to switch to costs you more than leaving a few points unspent: an account driven to its weekly limit leaves the rotation pool for days, while one that empties its 5-hour window is back within hours. When nothing is under its limit it spends reserve, and asks first whether a reserve can carry you until the fleet next has a target: an account that can outranks one that merely returns sooner, since it removes the gap outright. Reserves are compared in one unit, because a point of a weekly window is worth `autoswitch.windowRatio` points of a five-hour one (measurable: while an account is active both windows rise, and the ratio of the slopes is this number), and `autoswitch.burnRate` converts a reserve into minutes. It holds where it is when that account offers at least as much work and comes back no later, and a reserve too thin to be worth the switch is taken only when there is nothing else. The moment any account is back under its limit, that account is taken and reserve stops being spent. Two accounts within `autoswitch.runwayTieBand` of each other count as tied on weekly budget, and the tie goes to the one whose 5-hour window has not started: working on it starts its clock and pins its next refresh earlier, which is free when the weekly figures are level and not worth it when they are not. Every switch it makes says why, in the log line and as a `note` field in the JSON event. With `cswap config set autoswitch.primeIdleClocks true` it will also move to an account whose 5-hour window has not started, purely to start it, but only when no other move is due: an idle clock schedules no refresh at all, and starting it costs a few minutes of that account's weekly quota. It then stays for `autoswitch.primeHoldSeconds` (90 by default), because the engine cannot cause a request: it hands the credential over and the window opens when the client next calls. Measured, a 16-second visit started nothing while five-minute visits started every time. If an attempt lands nothing it is retried, but only once a reading of that account has arrived since, and each miss doubles the wait for it, so an account nothing is calling costs a couple of switches rather than a stream of them. Seeing its window open clears the history. Idle clocks are started back to back rather than one per visit: returning to the best account between every pair doubles the switches and buys nothing, since that account is still there when the pass ends. A priming move and the move that leaves a primed account skip `autoswitch.cooldownSeconds`, since a pass is not flapping and a prime's purpose is served the moment the window opens; the exemption ends itself, because the move that leaves is not a prime. Below the threshold it moves only when another account offers at least twice the runway (`cswap config set autoswitch.runwayMargin 1.25` to hand over sooner), so a modest edge never costs you a switch, and the return trip cannot qualify. Where seats differ in size, tell it so with `cswap config set autoswitch.accountWeights 6:5` (slot 6 holds five times what an unlisted slot holds): percentages are reported against each account's own limit, so without a weight a large seat reading 87% used looks nearly spent while it still holds more than a small one reading 4%. The weight also scales that account's `windowThresholds`, since a limit is a reserve and a reserve written in points is a different amount of work on every seat: with `7d:97` and `5h:92`, a 5x seat is judged at 99 and 98, holding back close to the same absolute capacity, and close to the same minutes of warning before its 5-hour window cuts off. Limits round down to whole percent, since that is the resolution the usage API reports: an unrounded 99.4 could only be crossed by a reading of 100, which already means exhausted, so the limit would never step the account aside.
...
cswap config set autoswitch.accountWeights 6:5            # slot 6 is a 5x-capacity seat
